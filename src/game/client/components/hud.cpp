...
void CHud::RenderScoreHud()
{
	...
	if(!(m_pClient->m_Snap.m_pGameData->m_GameStateFlags&(GAMESTATEFLAG_ROUNDOVER|GAMESTATEFLAG_GAMEOVER)))
	{
		...
		if(GameFlags&GAMEFLAG_TEAMS && m_pClient->m_Snap.m_pGameDataTeam && !(GameFlags&GAMEFLAG_RACE))
		{
			...
			for(int t = 0; t < NUM_TEAMS; t++)
			{
				...
				if(GameFlags&GAMEFLAG_SURVIVAL)
				{
					...
					TextRender()->Text(0, minimum(Whole-w-1.0f, Whole-ScoreWidthMax-ImageSize-2*Split), StartY+(t+1)*TeamOffset-3.0f, 8.0f, aBuf, -1.0f);
				}
				StartY += 8.0f;
			}
		...
		else
		{
			...
			float ScoreWidthMax = maximum(maximum(aScoreWidth[0], aScoreWidth[1]), TextRender()->TextWidth(0, FontSize, "10", -1, -1.0f));
			...
			if(GameFlags&GAMEFLAG_SURVIVAL)
			{
				...
				float w = TextRender()->TextWidth(0, 8.0f, aBuf, -1, -1.0f);
				TextRender()->Text(0, minimum(Whole - w - 1.0f, Whole - ScoreWidthMax - ImageSize - 2 * Split), StartY - 12.0f, 8.0f, aBuf, -1.0f);
			}
