...
// str_replace written by jmucchiello
...
{
	char *pOrig = malloc(orig_size);
	char *pRep = malloc(rep_size);
	char *pWith = malloc(with_size);
	char *tmp;
	str_copy(pOrig, orig, orig_size);
	str_copy(pRep, rep, rep_size);
	str_copy(pWith, with, with_size);
	tmp = str_replace_malloc(pOrig, pRep, pWith);
...
int escape_filename(char *buf, int buf_len, const char *str)
{
	...
	curl_free(pEsc);
	pTmp = str_replace_malloc(buf, ".", "%2E");
	str_copy(buf, pTmp, buf_len);
	if (pTmp != NULL)
		free(pTmp);
	pTmp = str_replace_malloc(buf, "~", "%7E");
	str_copy(buf, pTmp, buf_len);
	if (pTmp != NULL)
		free(pTmp);
	return 0;
